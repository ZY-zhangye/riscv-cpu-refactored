...
4. 等当前 L3D 125 MHz 稳健档签核完成后，再决定是否建立独立的旁路综合候选版本。
5. 旁路候选只有在 post-route Fmax 高于约 122.4 MHz 时才具备理论净收益；工程上建议至少通过 125 MHz，且完整功能回归无回退。

如果 125 MHz 无法收敛，直接放弃同周期旁路，不继续细分更多规则；后续转向分支预测或不增加 EX 组合深度的吞吐优化。
