...
3. 下一轮优先做分支预测仿真 A/B，选择不增加 EX 组合深度的表项、索引或恢复优化。
4. 对任何候选继续执行 125/130 MHz 双频门槛；125 MHz 必须保持明确正余量，130 MHz 仅作为性能上限尝试。
