// Standalone RV32M multiply/divide integration IP.
// This copy intentionally has no dependency on the CPU's defines.svh.
module rv32m_mul_div_ip #(
    parameter integer MUL_HIGH_PIPE_STAGES = 3,
    // 1: divider result is {remainder, quotient} (the bundled model).
    // 0: divider result is {quotient, remainder} (Vivado Divider Generator).
    parameter bit DIVIDER_RESULT_REMAINDER_HIGH = 1'b1
) (
    input logic clk,
    input logic rst_n,
    input logic is_mul,
    ...
    logic        s1,s2;
    logic [31:0] div_quotient_word;
    logic [31:0] div_remainder_word;
    ...
    assign div_quotient_word = DIVIDER_RESULT_REMAINDER_HIGH ?
                               div_result[31:0] : div_result[63:32];
    assign div_remainder_word = DIVIDER_RESULT_REMAINDER_HIGH ?
                                div_result[63:32] : div_result[31:0];
    ...
    rv32m_mul_pipeline_ip #(
        .HIGH_PIPE_STAGES(MUL_HIGH_PIPE_STAGES)
    ) u_mul_pipeline (
        .clk(clk),
        .rst_n(rst_n),
        .start(mul_start),
        ...
        unique case (1'b1)
            ...
            mul_op[1]: begin
                mul_result = (s1 && div_state == 2'b11) ?
                             (~div_quotient_word + 1'b1) : div_quotient_word;
            end
            mul_op[0]: begin
                mul_result = (s2 && div_state == 2'b11) ?
                             (~div_remainder_word + 1'b1) : div_remainder_word;
            end
            default: begin
                mul_result = 32'b0;
            end
            ...
            unique case (div_state)
                2'b00: begin
                    ...
                    if (div_start) begin
                        ...
                        div_state <= 2'b10;
                        div_result <= div_0 ?
                                      (DIVIDER_RESULT_REMAINDER_HIGH ?
                                       {mul_src1, 32'hffff_ffff} :
                                       {32'hffff_ffff, mul_src1}) :
                                      (DIVIDER_RESULT_REMAINDER_HIGH ?
    ...
    rv32m_divider_model_ip div_inst (
        .aclk(clk),
        .aresetn(rst_n),
