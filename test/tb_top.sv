...
module tb_uart_benchmark;
    ...
    localparam logic [31:0] PERF_MAGIC = 32'h4C33_4930; // "L3I0"
    localparam integer PERF_REPORT_WORDS = 20;
    localparam integer PERF_REPORT_COUNT = 8;
...
`endif
        ...
        if (!rst_n) begin
            ...
            if (u_my_cpu.u_cpu_top.u_regfile_csr.perf_enable && !perf_enable_d) begin
                if (perf_phase >= 0) begin
                    $display("L3I_WINDOW_START phase=%0d fetch_pc=%08h wb_pc=%08h",
            ...
            if (!u_my_cpu.u_cpu_top.u_regfile_csr.perf_enable && perf_enable_d) begin
                if (perf_phase >= 0) begin
                    $display("L3I_WINDOW_END phase=%0d fetch_pc=%08h wb_pc=%08h",
                             perf_phase, debug_inst_pc, debug_wb_pc);
                    $display("L3I_QUEUE phase=%0d depth0=%0d depth1=%0d depth2=%0d depth3=%0d depth4=%0d downstream_blocked=%0d qfull_downstream=%0d qfull_capacity=%0d",
                             perf_phase,
    ...
    task automatic display_report(input string report_name, input integer offset);
        ...
        begin
            ...
            $display("L3I_REPORT name=%s cycles=%0d instret=%0d ipc_x1000=%0d branch=%0d brmisp=%0d loaduse=%0d exstall=%0d dual=%0d single=%0d raw=%0d waw=%0d struct=%0d lsu_pair=%0d lane1_ctrl=%0d lsu_conflict=%0d bitman_pair=%0d cross_packet=%0d qfull=%0d exceptions=%0d",
    ...
    initial begin
        ...
        if (perf_word(0) !== PERF_MAGIC) begin
            $display("[TB] Timeout waiting for L3I performance mailbox.");
`ifdef DEBUG_EN
...
`endif
        ...
        $display("L3I_HEADER version=%0d cpu_freq_hz=%0d sink=%08h sim_cycles=%0d",
                 perf_word(1), perf_word(2), perf_word(3), wait_cycles);
        if ((perf_word(1) != 4) || (perf_word(2) == 0)) begin
            failures = failures + 1;
        end

        display_report("ALU", 4);
        display_report("BRANCH_RANDOM", 4 + PERF_REPORT_WORDS);
        ...
        display_report("BRANCH_RETURN", 4 + (6 * PERF_REPORT_WORDS));
        display_report("MEMORY", 4 + (7 * PERF_REPORT_WORDS));
        ...
        $display("L3I_OVERALL cycles=%0d instret=%0d ipc_x1000=%0d",
