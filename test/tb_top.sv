...
module tb_uart_benchmark;
    ...
    localparam integer PERF_BASE_WORD = 448; // 0x6000_0700映射到data RAM word 448
    localparam logic [31:0] PERF_MAGIC = 32'h4C33_4530; // "L3E0"
...
`endif
        ...
        if (!rst_n) begin
            ...
            if (!u_my_cpu.u_cpu_top.u_regfile_csr.perf_enable && perf_enable_d) begin
                if (perf_phase >= 0) begin
                    $display("L3E_QUEUE phase=%0d depth0=%0d depth1=%0d depth2=%0d depth3=%0d depth4=%0d downstream_blocked=%0d qfull_downstream=%0d qfull_capacity=%0d",
                             perf_phase,
    ...
    task automatic display_report(input string report_name, input integer offset);
        ...
        begin
            ...
            $display("L3E_REPORT name=%s cycles=%0d instret=%0d ipc_x1000=%0d branch=%0d brmisp=%0d loaduse=%0d exstall=%0d dual=%0d single=%0d raw=%0d waw=%0d struct=%0d lsu_pair=%0d lane1_ctrl=%0d lsu_conflict=%0d bitman_pair=%0d cross_packet=%0d qfull=%0d exceptions=%0d",
    ...
    initial begin
        ...
        if (perf_word(0) !== PERF_MAGIC) begin
            $display("[TB] Timeout waiting for L3E performance mailbox.");
`ifdef DEBUG_EN
...
`endif
        ...
        $display("L3E_HEADER version=%0d cpu_freq_hz=%0d sink=%08h sim_cycles=%0d",
                 perf_word(1), perf_word(2), perf_word(3), wait_cycles);
        if ((perf_word(1) != 1) || (perf_word(2) == 0)) begin
            failures = failures + 1;
        end

        display_report("ALU", 4);
        display_report("BRANCH", 4 + PERF_REPORT_WORDS);
        display_report("MEMORY", 4 + (2 * PERF_REPORT_WORDS));

        total_cycles = perf_word(4) +
                       perf_word(4 + PERF_REPORT_WORDS) +
                       perf_word(4 + (2 * PERF_REPORT_WORDS));
        total_instret = perf_word(5) +
                        perf_word(5 + PERF_REPORT_WORDS) +
                        perf_word(5 + (2 * PERF_REPORT_WORDS));
        total_ipc_x1000 = (total_instret * 1000) / total_cycles;
        $display("L3E_OVERALL cycles=%0d instret=%0d ipc_x1000=%0d",
